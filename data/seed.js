 ...
 * - Adds BULK UPDATE for analytics ('updateAnalyticsBulk')
 * - Adds small helper to BULK INSERT rows ('insertMany') when needed
 *
 * NOTE:
 * - We cast ENUMs/JSONB explicitly to avoid 'could not determine data type of parameter $X' errors.
 * - This script is idempotent-ish (ON CONFLICT where possible).
 */
 ...
 *   If not provided, no casts are appended.
 */
...
async function insertResource(client, type) {
  const { rows } = await client.query(
    `INSERT INTO resources(mime_type, resource_type, url, public_id, name)
     VALUES ($1, $2::resource_type, $3, $4, $5)
     RETURNING id`,
    [
...
/**
 * updateAnalyticsBulk(client, updates)
 * updates: Array<{ id: number, event_type?, target_id?, metadata?, occurred_at? }>
 * - Any field not provided for an item will be left unchanged (via COALESCE).
 * - Uses explicit casts to match V1__init.sql types.
 */
async function updateAnalyticsBulk(client, updates) {
  ...
  // Flatten params and build placeholders
  const values = [];
  let i = 1;
  const tuples = rows.map(r => {
    const tuple = `($${i++}::bigint, $${i++}::event_type, $${i++}::bigint, $${i++}::jsonb, $${i++}::timestamptz)`;
    // Push JSON string for metadata if object
    values.push(
      r[0],
      r[1],
      r[2],
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < candidateAccountPool.length; i++) {
      ...
      const { rows } = await client.query(
        `INSERT INTO candidates(
         ...
         VALUES ($1, $2, $3, $4, $5::seniority_level, $6, $7, $8, $9, $10, $11, $12)
    ...
    const roleSeeds = [
      'Software Engineer','Backend Developer','Frontend Developer','DevOps Engineer','QA Engineer','Data Engineer','Data Scientist','AI Engineer','Mobile Developer','Fullstack Developer','Product Manager','UI/UX Designer','Solution Architect','Blockchain Engineer',
    ];
    ...
    /* =========================================
     * analytics (INSERT) — cast JSONB explicitly
     * ========================================= */
    const analyticsRows = [];
     ...
     * - Randomly pick ~50 analytics rows and tweak their metadata/occurred_at
     * ========================================= */
